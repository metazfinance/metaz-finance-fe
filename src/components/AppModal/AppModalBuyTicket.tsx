...
export const AppModalBuyTicket2 = ({ onClose }: { onClose: () => void }) => {
  const { isFetching, balance } = useBalanceErc20(
    contractAddress.ERC20_LOTTERY_V2
  );

  const { data: currentLotteryInfo } = useGetCurrentLotteryInfo2();
  const { buyTickets } = useActionLottery2();
  const { isApprove, isApproving, approve } = useApproveERC20(
