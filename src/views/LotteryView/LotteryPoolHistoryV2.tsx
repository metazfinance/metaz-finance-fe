...
import { AppCheckClaimable2 } from "./components/AppCheckClaimable";

export default function LotteryPoolHistory2() {
  const provider = useGetProvider();
  const { data } = useLottery2();
  const { data: _lastLottery } = useGetCurrentLotteryInfo2();
  const { getLotteryInfo, getRewardAccount } = useGetLotteryInfo2();
